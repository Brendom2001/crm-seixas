...
export default function KanbanCard({ lead, onEdit, onDelete, accentColor }) {
  ...
  return (
    <div
      ...
      onClick={() => onEdit(lead)}
      className={`
        bg-[#161616] border border-[#202020] rounded-xl p-3.5 cursor-pointer group shadow-sm select-none animate-fade-up
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-white text-sm font-semibold leading-snug line-clamp-1">{lead.nome}</p>
            <p className="text-[#7b7b7b] text-[11px] truncate">{lead.tipo_negocio || '—'}</p>
          </div>
        </div>
        ...
        <button
          ...
          className="opacity-0 group-hover:opacity-100 w-7 h-7 flex items-center justify-center rounded-md text-[#7b7b7b] hover:text-red-400 hover:bg-red-400/10 transition-all flex-shrink-0"
        >
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <span
          className="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-[#2a2a2a] text-[11px] text-[#ccc]"
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" className="text-[#f97316]">
            <path d="M5 3h14v4H5V3zm0 6h14v12H5V9z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
          {lead.tipo_negocio || 'Sem tipo'}
        </span>
        <span
          className="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-[#2a2a2a] text-[11px] text-[#ccc]"
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" className="text-[#60a5fa]">
        ...
        {formatted && (
          <span
            className="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-[#2a2a2a] text-[11px] text-[#ccc]"
          >
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" className="text-[#4ade80]">
